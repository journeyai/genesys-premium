...
const Validator = {
  ...
  propertyExists(obj, propertyName, objectName, additionalComment) {
    if (propertyName in obj) {
      ...
      if (
        ...
        obj[propertyName].trim().length <= 0
      ) {
        return Promise.resolve([
          false,
          `${propertyName} does not exist in ${objectName}. -- ${additionalComment}`,
        ]);
      }

      return Promise.resolve([
        true,
        `${propertyName} exists in ${objectName}. -- ${additionalComment}`,
      ]);
    }
  ...
  notEqual(value1, value2, value1Name, additionalComment) {
    ...
    if (value1 !== value2) {
      return Promise.resolve([
        true,
        `${value1Name} is not equal to '${value2.toString()}' -- ${additionalComment}`,
      ]);
    } else {
      return Promise.resolve([
        false,
        `${value1Name} is equal to '${value2.toString()}' -- ${additionalComment}`,
      ]);
    }
  },
  ...
  customEvaluation(func, passMessage, failMessage, additionalComment) {
    ...
    if (typeof result != "boolean")
      throw new Error("Func does not return boolean");
  ...
  async customEvaluationAsync(
    ...
    if (typeof result != "boolean")
      throw new Error("Func resolution does not return boolean");
...
function printMessages() {
  ...
  if (!passedMessages || passedMessages.length <= 0)
  ...
  if (!warningMessages || warningMessages.length <= 0)
    console.log(chalk.grey("none"));
  warningMessages.forEach((m, i) =>
    console.log(chalk.yellow(`${i + 1}. ${m}`))
  );
  console.log();
  ...
  if (!criticalMessages || criticalMessages.length <= 0)
    console.log(chalk.grey("none"));
  criticalMessages.forEach((m, i) =>
    console.log(chalk.redBright(`${i + 1}. ${m}`))
  );
  console.log();

  console.log(
    chalk.black.bgWhite.underline(
      "NOTE: Some warnings are acceptable especially if the evaluation is done prior to a demo."
    )
  );
  console.log(
    chalk.black.bgWhite.underline(
      "For production-ready wizards, every test should pass."
    )
  );
  console.log(
    chalk.black.bgWhite.underline(
      "If there are any questions, please contact your Genesys Developer Evangelist POC."
    )
  );
  console.log();
}
...
async function validateConfig() {
  ...
  await Validator.evaluateArr(Validator.WARNING, [
    ...
    Validator.customEvaluation(
      ...
      "wizardUriBase is not localhost",
      "wizardUriBase is localhost",
      "wizardUriBase should be a publically available URL"
    ),
    Validator.customEvaluation(
      () => {
      ...
      "redirectURLOnWizardCompleted is not localhost",
      "redirectURLOnWizardCompleted is localhost",
      "redirectURLOnWizardCompleted should be a publically available URL"
    ),

    // Integration Type ID
    Validator.notEqual(
      config.premiumAppIntegrationTypeId,
      defaultIntegrationTypeId,
      "premiumAppIntegrationTypeId",
      "Once integration is approved in AppFoundry, premiumAppIntegrationTypeId should match the provided unique ID."
    ),
    ...
    Validator.notEqual(
      ...
      "Once integration is approved in AppFoundry, premiumAppViewPermission should match the new unique permission."
    ),
  ]);
  ...
  await Validator.evaluateArr(Validator.CRITICAL, [
    ...
    Validator.notEqual(
      ...
      defaultClientId,
      "clientID",
      "clientID should be replaced with your own client ID."
    ),
    Validator.propertyExists(
      config,
      "clientID",
      "config",
      "ClientID should exist"
    ),
    Validator.propertyExists(
      config,
      "wizardUriBase",
      "config",
      "wizardUriBase should exist"
    ),
    Validator.propertyExists(
      config,
      "redirectURLOnWizardCompleted",
      "config",
      "redirectURLOnWizardCompleted should exist"
    ),
    Validator.propertyExists(
      config,
      "premiumAppIntegrationTypeId",
      "config",
      "premiumAppIntegrationTypeId should exist"
    ),
    Validator.propertyExists(
      config,
      "premiumAppViewPermission",
      "config",
      "premiumAppViewPermission should exist"
    ),
    ...
    Validator.customEvaluation(
      () => {
        ...
        if (["all", "premium", "wizard", "none"].includes(installPermisison))
          return true;
    ...
    Validator.propertyExists(
      ...
      "config",
      "defaultGcEnvironment should exist"
    ),
    ...
    Validator.notEqual(
      ...
      defaultPrefix,
      "prefix",
      "Prefix should be updated to be unique"
    ),
    Validator.propertyExists(
      config,
      "provisioningInfo",
      "config",
      "provisioningInfo should exist"
    ),
    Validator.propertyExists(
      config,
      "defaultLanguage",
      "config",
      "defaultLanguage should exist"
    ),
    Validator.propertyExists(
      config,
      "availableLanguageAssets",
      "config",
      "availableLanguageAssets should exist"
    ),
    ...
    Validator.customEvaluation(
      () => {
        ...
        return Object.keys(config.availableLanguageAssets).includes(
    ...
    Validator.propertyExists(
      ...
      "installPermissions",
      "config",
      "installPermissions should exist"
    ),
    Validator.propertyExists(
      config,
      "uninstallPermissions",
      "config",
      "uninstallPermissions should exist"
    ),
    Validator.propertyExists(
      config,
      "installScopes",
      "config",
      "installScopes should exist"
    ),
    Validator.propertyExists(
      config,
      "uninstallScopes",
      "config",
      "uninstallScopes should exist"
    ),
    Validator.notEqual(
      config.enableUninstall,
      true,
      "config.enableUninstall",
      "Uninstall should be disabled for production"
    ),
  ]);
}
...
async function validateLanguageFiles() {
  ...
  Object.keys(config.availableLanguageAssets).forEach((langKey) => {
    toBeEvaluated.push(
      Validator.customEvaluationAsync(
        async () => {
...
async function validateWizardText() {
  ...
  try {
    const langFileData = await fs.readFile(
      path.join(languageDirPath, "en-us.json")
    );
  ...
  Object.keys(defaultLanguage).forEach((textKey) => {
    forEvaluation.push(
      Validator.notEqual(
        langFileObject[textKey],
        defaultLanguage[textKey],
        textKey,
        `text should be personalized`
      )
    );
  });
...
async function validateStyles() {
  ...
  try {
    const cssData = await fs.readFile(
      path.join(wizardPath, "styles/style.css")
    );
  ...
  if (titleColor) {
    forEvaluation.push(
      Validator.notEqual(titleColor, "#3B90AA", ".title CSS", "Personalize CSS")
    );
  }
