...
async function getExisting() {
  ...
  let _getIntegrations = async (pageNum) => {
    ...
    data.entities
      .filter((entity) => {
        return (
          entity.integrationType.id == "custom-rest-actions" &&
          entity.name.startsWith(config.prefix)
        );
      })
      .forEach((integration) => integrations.push(integration));
    ...
    instances.forEach((entity) => {
      del_ws.push(
        (async () => {
          ...
          if (
            entity.config.current.credentials &&
            entity.config.current.credentials.basicAuth &&
            entity.config.current.credentials.basicAuth.id
          ) {
            return integrationsApi.deleteIntegrationsCredential(
              entity.config.current.credentials.basicAuth.id
            );
          }
        })()
...
async function configure(logFunc, installedData, userId) {
  ...
  Object.keys(wsInstancesData).forEach((instanceKey) => {
    ...
    let wsInstanceInstall = instanceInstallationData.find(
      (a) => a.name == instanceKey
    );
    ...
    let integrationConfig = {
      body: {
        ...
        notes:
          wsInstanceInstall.notes ||
          `Provisioned by ${config.premiumAppIntegrationTypeId} integration`,
        credentials: {},
      },
    };
    ...
    let wsCredentialsPromise = new Promise(async (resolve, reject) => {
      if (
        wsInstanceInstall.credentialType &&
        (wsInstanceInstall.credentialType === "userDefinedOAuth" ||
          wsInstanceInstall.credentialType === "userDefined" ||
          wsInstanceInstall.credentialType === "basicAuth")
      ) {
        try {
          let data = await integrationsApi.postIntegrationsCredentials(
            credentialsConfig
          );
          resolve(data.id);
        } catch (e) {
    ...
    promisesArr.push(
      (async () => {
        ...
        if (
          wsInstanceInstall.autoEnable &&
          wsInstanceInstall.autoEnable === true
        ) {
          let opts = {
            body: {
        ...
        if (
          wsInstanceInstall["data-actions"] &&
          wsInstanceInstall["data-actions"].length <= 0
        ) {
          return;
        }
        ...
        wsInstanceInstall["data-actions"].forEach((dataAction) => {
          ...
          promiseDataActions.push(
            (async () => {
              let createDataActionsResult =
                await integrationsApi.postIntegrationsActionsDrafts(
                  dataActionBody
                );
              logFunc(
                "Created Web Services Data Action: " +
                  createDataActionsResult.name
              );
              ...
              let patchDataActionsResult =
                await integrationsApi.patchIntegrationsActionDraft(
                  dataActionUpdateBody.id,
                  dataActionUpdateBody
                );
              if (dataAction.autoPublish && dataAction.autoPublish === true) {
                return integrationsApi.postIntegrationsActionDraftPublish(
                  patchDataActionsResult.id,
                  patchDataActionsResult
                );
              } else {
                return;
              }
        ...
        if (
          wsInstanceInstall.credentialType &&
          wsInstanceInstall.credentialType == "userDefinedOAuth" &&
          wsInstanceInstall.customAuthAction &&
          wsInstanceInstall.customAuthAction.update &&
          wsInstanceInstall.customAuthAction.update === true
        ) {
          // Set back to draft
          let createDraftResult =
            await integrationsApi.postIntegrationsActionDraft(
              "customAuth_-_" + wsInstance.id
            );
          let dataActionUpdateBody = { ...createDraftResult };
          dataActionUpdateBody.config =
            wsInstanceInstall.customAuthAction.config;
          delete dataActionUpdateBody.contract;
          delete dataActionUpdateBody.secure;

          let patchResults = await integrationsApi.patchIntegrationsActionDraft(
            dataActionUpdateBody.id,
            dataActionUpdateBody
          );
