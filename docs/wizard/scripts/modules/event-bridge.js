...
async function getExisting() {
  ...
  let _getIntegrations = async (pageNum) => {
    ...
    data.entities
      .filter((entity) => {
        return (
          entity.integrationType.id == "amazon-eventbridge-source" &&
  ...
  data.forEach((instance) => {
    ...
    integrationPromises.push(
      (async () => {
        ...
        logFunc(
          "Created Genesys Cloud AWS EventBridge integration: " + instance.name
        );
    ...
    promisesArr.push(
      (async () => {
        try {
          ...
          logFunc(
            "Configured Genesys Cloud AWS EventBridge integration: " +
              gcInstance.name
          );
          if (
