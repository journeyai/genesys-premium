...
const telephonyProvidersEdgeApi =
  new platformClient.TelephonyProvidersEdgeApi();
...
async function getExisting() {
  ...
  let _getTrunks = async (pageNum) => {
    let data =
      await telephonyProvidersEdgeApi.getTelephonyProvidersEdgesTrunkbasesettings(
        {
          pageSize: 100,
          pageNumber: pageNum,
          name: config.prefix + "*",
    ...
    if (data.pageCount > 0) {
      data.entities
        .filter((tr) => tr.name.startsWith(config.prefix))
        .forEach((trunk) => trunks.push(trunk));
...
async function remove(logFunc) {
  ...
  if (instances.length > 0) {
    instances.forEach((entity) => {
      del_trunks.push(
        telephonyProvidersEdgeApi.deleteTelephonyProvidersEdgesTrunkbasesetting(
          entity.id
        )
      );
    });
      ...
      .then((currentOrg) => {
        telephonyProvidersEdgeApi
          ...
          .then((trunkTemplate) => {
            ...
            trunkTemplate.properties.trunk_label.value.instance =
              config.prefix + tr.name;
            trunkTemplate.properties.trunk_sip_termination_uri.value.instance =
              tr.inboundSIPTerminationIdentifier &&
              tr.inboundSIPTerminationIdentifier != ""
                ? currentOrg.thirdPartyOrgName +
                  tr.inboundSIPTerminationIdentifier
                : "";
            Object.keys(tr.properties).forEach((propKey) => {
              trunkTemplate.properties[propKey].value.instance =
                tr.properties[propKey];
            });
            trunkPromises.push(
              (async () => {
                let result =
                  await telephonyProvidersEdgeApi.postTelephonyProvidersEdgesTrunkbasesettings(
                    trunkTemplate
                  );
