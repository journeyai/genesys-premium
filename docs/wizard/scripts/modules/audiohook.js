...
async function getExisting() {
  ...
  let _getIntegrations = async (pageNum) => {
    ...
    data.entities
      .filter((entity) => {
        return (
          entity.integrationType.id == "audiohook" &&
      ...
      del_ah.push(
        (async () => {
          ...
          if (
            ...
            entity.config.current.credentials.audioHook &&
            entity.config.current.credentials.audioHook.id
          ) {
  ...
  data.forEach((instance) => {
    ...
    integrationPromises.push(
      (async () => {
        ...
        logFunc(
          "Created Genesys Cloud AudioHook integration: " + instance.name
        );
...
async function configure(logFunc, installedData, userId) {
  ...
  Object.keys(gcInstancesData).forEach((instanceKey) => {
    ...
    let integrationConfig = {
      body: {
        ...
        properties: {
          ...
          connectionUri:
            gcInstanceInstall.connectionUri || "wss://test.test.test",
        },
    ...
    promisesArr.push(
      (async () => {
        let credResult = await integrationsApi.postIntegrationsCredentials(
          credentialsConfig
        );
        ...
        logFunc(
          "Configured Genesys Cloud AudioHook integration: " + gcInstance.name
        );
        if (
