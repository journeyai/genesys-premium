...
async function getExisting() {
  ...
  let _getOpenMessagingIntegrations = async (pageNum) => {
    ...
    if (data.pageCount > 0) {
      data.entities
        .filter((omi) => omi.name.startsWith(config.prefix))
...
async function remove(logFunc) {
  ...
  if (instances.length > 0) {
    instances.forEach((entity) => {
      del_integrations.push(
        conversationsApi.deleteConversationsMessagingIntegrationsOpenIntegrationId(
          entity.id
        )
      );
    });
  ...
  data.forEach((instance) => {
    let integrationBody = {
      ...
      outboundNotificationWebhookUrl:
        instance.outboundNotificationWebhookUrl ||
        "https://yourservice.com/messages",
      outboundNotificationWebhookSignatureSecretToken:
        instance.outboundNotificationWebhookSignatureSecretToken ||
    ...
    integrationPromises.push(
      (async () => {
        let result =
          await conversationsApi.postConversationsMessagingIntegrationsOpen(
            integrationBody
          );
