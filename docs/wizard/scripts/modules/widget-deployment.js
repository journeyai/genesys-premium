...
async function configure(logFunc, installedData, userId) {
  ...
  Object.keys(deploymentInstancesData).forEach((instanceKey) => {
    ...
    let deploymentInstanceInstall = instanceInstallationData.find(
      (a) => a.name == instanceKey
    );

    let deploymentConfig = deploymentInstance;
    if (
      deploymentInstanceInstall.autoEnable &&
      deploymentInstanceInstall.autoEnable === true
    ) {
      deploymentConfig.disabled = false;

      promisesArr.push(
        (async () => {
          await widgetsApi.putWidgetsDeployment(
