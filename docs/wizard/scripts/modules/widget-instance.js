...
async function getExisting() {
  ...
  let _getIntegrations = async (pageNum) => {
    ...
    data.entities
      .filter((entity) => {
        return (
          entity.integrationType.id == config.premiumWidgetIntegrationTypeId &&
...
async function create(logFunc, data) {
  ...
  return Promise.all(integrationPromises).then(() => integrationsData);
}
...
async function configure(logFunc, installedData, userId) {
  ...
  Object.keys(appInstancesData).forEach((instanceKey) => {
    ...
    let appInstanceInstall = instanceInstallationData.find(
    ...
    let integrationConfig = {
      body: {
        ...
        properties: {
          ...
          queueIdFilterList: [],
          communicationTypeFilter: appInstanceInstall.communicationTypeFilter
            ? appInstanceInstall.communicationTypeFilter
            : "",
          groups: appInstanceInstall.groups
            .map((groupName) => installedData.group[groupName].id)
            .filter((g) => g != undefined),
        },
        advanced: appInstanceInstall.advanced || {},
        notes:
          appInstanceInstall.notes ||
    ...
    if (config.premiumAppIntegrationTypeId === "premium-widget-example") {
      integrationConfig.body.properties.sandbox =
        appInstanceInstall.sandbox ||
        "allow-forms,allow-modals,allow-popups,allow-presentation,allow-same-origin,allow-scripts,allow-downloads";
      integrationConfig.body.properties.permissions =
        appInstanceInstall.permissions ||
    ...
    promisesArr.push(
      (async () => {
        try {
          ...
          let data = await integrationsApi.patchIntegration(
            appInstance.id,
            opts
          );
