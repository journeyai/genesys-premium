...
async function getExisting() {
  ...
  let _getIntegrations = async (pageNum) => {
    ...
    data.entities
      .filter((entity) => {
        return (
          entity.integrationType.id == "purecloud-data-actions" &&
      ...
      del_gc.push(
        (async () => {
          ...
          if (
            ...
            entity.config.current.credentials.pureCloudOAuthClient &&
            entity.config.current.credentials.pureCloudOAuthClient.id
          ) {
...
async function configure(logFunc, installedData, userId) {
  ...
  Object.keys(gcInstancesData).forEach((instanceKey) => {
    ...
    let gcInstanceInstall = instanceInstallationData.find(
    ...
    promisesArr.push(
      (async () => {
        ...
        if (
          gcInstanceInstall.autoEnable &&
          gcInstanceInstall.autoEnable === true
        ) {
          let opts = {
            body: {
        ...
        if (
          gcInstanceInstall["data-actions"] &&
          gcInstanceInstall["data-actions"].length <= 0
        ) {
          return;
        }
        ...
        gcInstanceInstall["data-actions"].forEach((dataAction) => {
          ...
          promiseDataActions.push(
            (async () => {
              ...
              logFunc(
                "Created Genesys Cloud Data Action: " +
